...
import{ fertImages} from './imagelinks';
...
function FertilizerPrediction() {
    ...
    const handlePredict = async () => {
        try {
            // Make a request to your backend for fertilizer predictions
            const response = await axios.post('http://localhost:5000/api/predict/fertilizer', inputValues);
    ...
    const getImageUrl = () => {
        ...
        // console.log("Image URL:", fertImages[prediction]);
        return fertImages[prediction];
    };
    return (
        <div className='min-h-screen bg-bgfert bg-cover bg-center flex flex-col md:flex-row items-center justify-center gap-8'>
            <div className='min-h-screen flex flex-col items-center justify-center gap-8'>
                <h1 className='text-xl sm:text-3xl font-extrabold text-white '>Fertilizer Prediction 🌱</h1>
                ...
                {prediction && (
                    <div className="flex flex-col sm:w-[70%] md:w-full mx-auto bg-white border border-gray-200 rounded-lg shadow md:flex-row md:max-w-xl hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">
                        ...
                        <div className="flex flex-col justify-between p-4 leading-normal">
                            ...
                            <p className="mb-3 font-normal text-gray-700 dark:text-gray-400"> Explore the benefits of using {prediction} fertilizer for your crops. Ensure optimal growth and yield with this recommended fertilizer. 🌿 </p>
                        </div>
                    </div>
                )}
            </div>
